...
#define ConsoleTitle _T("racaljk-host tool    %s  Build time: %s")
...
#define VERSION _T("v2.1.19")

//main entry
int _tmain(int argc, TCHAR const **argv) {
    TCHAR consoleTitle[64];
    _stprintf(consoleTitle, ConsoleTitle, VERSION, get_current_time());
...
char *get_current_time() {
    ...
    static char tmp[64];
    strftime(tmp, sizeof(tmp), "%b. %dth %Y %X", localtime(&time1));
    return tmp;
}
