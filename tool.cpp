...
void Func_ResetFile(bool silent);
...
SERVICE_TABLE_ENTRY STE[2] = {{Sname, Service_Main},
                              {_pNULL_, _pNULL_}};
...
int __fastcall __Check_Parameters(int argc, TCHAR const **argv) {
    ...
    switch (i) {
        ...
        case 1:
            return EXEC_START_INSTALL_SERVICE;
        case 2:
            return EXEC_START_UNINSTALL_SERVICE;
        case 4:
            return EXEC_DEBUG_RESET;    //restart service
        case 5:
            return EXEC_START_INSTALL2;
        case 6:
            return EXEC_START_HELP;
        case 7:
            return DEBUG_SERVICE_STOP;    //stop service
        case 8:
            return DEBUG_SERVICE_START;    //start service
        case 9:
            return DEBUG_SERVICE_REINSTALL;//reinstall service
        case 10:
            return OPEN_LISTEN;
        case 12:
            return RESET_FILE;
        case 13:
            return EXEC_HIDDEN_START;
        case 14:
            return EXEC_HIDDEN_STARTED;
        default:
            BAD_EXIT;
    }
    BAD_EXIT;
...
//			CASE(SHOW_LICENSE,__show_str(szgpl_Raw,_pNULL_));
            ...
            CASE(RESET_FILE, Func_ResetFile(false));
...
void Func_ResetFile(bool silent) {
    ...
    bool chk = false;
    if (!silent) _tprintf(copyrightshow);
...
\tCannot get system path!"), GetLastError()), abort();
    ...
    if (CopyFile(buf1, buf2, FALSE)) {
        if (!silent) _tprintf(_T("Backup File success\nFilename:%s\n\n"), buf2);
...
//reset file start
    ...
    if (!fp) {//,abort();
        if (request_client) {
            if (!silent) _tprintf(_T("Cannot open file!\nUsing plan B.\n"));
...
\"%%systemroot%%\\drivers\\etc\\hosts\" Manually."),
        ...
        fclose(fp);
        if (!silent) _tprintf(_T("    Reset file successfully.\n"));
        if (!silent) callsystempause;
        ExitProcess(0);
    }
...
Or open new issue\n------------------------------------------------------\n\n"));
    ...
    try {
        ...
        if (!request_client) {
            ...
            else if (!StartService(shSvc, 1, SzName))
...
void Func_CallCopyHostsFile(SYSTEMTIME &st) {
    ...
    if (!bIsServiceMode)
        Func_countBackupFile(&st), MessageBox(_pNULL_,
        ...
        try {
            ...
            try {
                ...
                if (!(fp = _tfopen(buf1, _T("r"))))
                    // file not found
                    if (!(fp = _tfopen(buf1, _T("at+"))))
                        throw buf1;
                    else
                        Func_ResetFile(true);
//				if(_fputts("# racaljk", fp) <= 0){
//					throw buf1;
//				}
